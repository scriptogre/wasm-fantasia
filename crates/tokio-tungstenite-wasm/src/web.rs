                ...
                Ok(ReadyState::Open) => {
                    match item {
                        ...
                        crate::Message::Binary(bin) => {
                            // Copy to an owned ArrayBuffer because send_with_u8_array
                            // creates a view into WASM linear memory, which fails when
                            // that memory is a SharedArrayBuffer (multi-threaded WASM).
                            let copy = js_sys::Uint8Array::new_with_length(bin.len() as u32);
                            copy.copy_from(&bin);
                            self.inner
