...
#[cfg(feature = "bevy")]
use crate::dynamics::solver::joint_graph::JointGraph;
use crate::prelude::*;
#[cfg(feature = "bevy")]
use bevy::{
...
/// A trait for constraints between entities (non-Bevy variant without MapEntities).
#[cfg(not(feature = "bevy"))]
pub trait EntityConstraint<const ENTITY_COUNT: usize> {
    /// The entities participating in the constraint.
    fn entities(&self) -> [Entity; ENTITY_COUNT];
}
...
#[cfg_attr(feature = "bevy", component(on_add = JointCollisionDisabled::on_add, on_remove = JointCollisionDisabled::on_remove))]
...
impl JointFrame {
    ...
    pub fn local(isometry: impl Into<Isometry>) -> Self {
        ...
        #[cfg(feature = "3d")]
        let anchor = VectorF32::from(isometry.translation).adjust_precision();
        Self {
