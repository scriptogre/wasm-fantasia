...
pub mod prelude {
    // Always-available physics types
    pub(crate) use crate::math::*;
    pub use crate::math::{Matrix, Quaternion, Scalar, Vector};
    #[cfg(feature = "bevy")]
    pub(crate) use crate::physics_transform::{PreSolveDeltaPosition, PreSolveDeltaRotation};
    pub use crate::{
    ...
    // Standalone PhysicsWorld API (always available; no conflict with bevy types).
    pub use crate::standalone::*;

    // When bevy is off, derive macros (Deref, DerefMut, From) come from derive_more
    // instead of bevy's prelude re-export.
    #[cfg(not(feature = "bevy"))]
    pub use derive_more::{Deref, DerefMut, From};

    // When bevy is off, log macros come from the log crate directly
    // instead of bevy's re-export.
    #[cfg(not(feature = "bevy"))]
    pub(crate) use log::warn;

    #[cfg(all(feature = "bevy", feature = "default-collider"))]
    ...
    pub use crate::{
        ...
        interpolation::*,
        physics_transform::{PhysicsTransformHelper, PhysicsTransformPlugin},
        schedule::{
    ...
    #[cfg(feature = "bevy")]
    pub(crate) use crate::{diagnostics::AppDiagnosticsExt, schedule::TimePrecisionAdjusted};
...
// Entity abstraction: use Bevy's Entity when available, standalone otherwise.
#[cfg(feature = "bevy")]
pub use bevy::ecs::entity::hash_set::EntityHashSet;
#[cfg(feature = "bevy")]
pub use bevy::ecs::entity::{Entity, EntityGeneration};

#[cfg(not(feature = "bevy"))]
mod entity_handle;
#[cfg(not(feature = "bevy"))]
pub use entity_handle::{Entity, EntityGeneration, EntityHashSet};
