...
"]
    ...
    /// WASM build: connect via spawn_local, no tokio runtime.
    /// Returns immediately with placeholder channels; the real WebSocket
    /// connects asynchronously and swaps in the real channels once ready.
    #[cfg(feature = "web")]
    fn build_impl(self) -> crate::Result<DbContextImpl<M>> {
        use crate::wasm_safe::MutexExt;
        ...
        // On WASM we can't block, so set up placeholder channels and connect asynchronously.
        let (pending_mutations_send, pending_mutations_recv) = mpsc::unbounded();
        let (placeholder_ws_send, placeholder_ws_recv) = mpsc::unbounded::<ws::v2::ClientMessage>();
