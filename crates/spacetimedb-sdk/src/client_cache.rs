...
impl<Row: Clone + Send + Sync + 'static> TableCache<Row> {
    ...
    pub fn add_unique_constraint<Col>(
        &mut self,
        unique_index_name: &'static str,
...
impl<Row: InModule + Send + Sync + Clone + 'static> TableHandle<Row> {
    ...
    fn with_table_cache<Res>(&self, get: impl FnOnce(&TableCache<Row>) -> Res) -> Res {
        let client_cache = self.client_cache.wasm_lock();
        client_cache
    ...
    pub fn on_update(
        &self,
        mut callback: impl FnMut(&<Row::Module as SpacetimeModule>::EventContext, &Row, &Row)
            + Send
            + 'static,
    ) -> CallbackId {
        let callback_id = CallbackId::get_next();
    ...
    > UniqueConstraintHandle<Row, Col>
{
    pub fn find(&self, col_val: &Col) -> Option<Row> {
        self.table_handle.with_table_cache(|table| {
            table
                .find_by_unique_index(self.unique_index_name, col_val)
                .cloned()
        })
    }
}
