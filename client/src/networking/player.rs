...
pub fn setup_remote_animations(
    ...
    for (remote_entity, mut remote_anim) in remote_q.iter_mut() {
        ...
        for (name, clip_handle) in gltf.named_animations.iter() {
            ...
            anim_map.insert(anim, node_index);
        }

        // Debug: warn about any expected animations missing from the model
        #[cfg(debug_assertions)]
        for anim in Animation::ALL {
            if !anim_map.contains_key(anim) {
                warn!("Animation {:?} ({}) not found in player model for remote player", anim, anim.clip_name());
            }
        }

        info!(
            "Initialized remote player animations ({} clips)",
            anim_map.len()
        );
...
pub fn animate_remote_players(
    ...
    for (entity, rp, mut remote_anim) in remote_q.iter_mut() {
        ...
        let (clip, speed, should_loop) = if new_attack {
            let Some(anim) = Animation::from_clip_name(&player.attack_anim) else {
                continue;
            };
            let speed = if anim == Animation::MeleeHook {
                1.1
            } else {
                1.3
            };
            ...
            match player.anim_state.as_str() {
                "Idle" => (Animation::Idle, 1.0, true),
                "Walk" => (Animation::JogFwd, 0.6, true),
                "Run" => (Animation::Sprint, 1.5, true),
                "Jump" => (Animation::JumpLoop, 0.5, true),
                "Crouch" => (Animation::CrouchIdle, 1.0, true),
                "Hit" => (Animation::HitChest, 1.0, false),
                "Slide" => (Animation::SlideLoop, 1.0, true),
                "Attack" => continue, // Wait for attack_seq to change
                _ => (Animation::Idle, 1.0, true),
            }
        };

        let Some(index) = remote_anim.animations.get(&clip) else {
            continue;
        };
        ...
        let active = transitions
            .play(&mut anim_player, *index, BLEND)
            .set_speed(speed);
        if should_loop {
            active.repeat();
        }
...
pub fn update_remote_players(
    ...
    for (rp, mut interp) in query.iter_mut() {
        ...
        } else if let Some(state) = buffers.inbound_buffer.get(&rp.identity) {
...
pub fn send_local_position(
    ...
    let (attack_seq, attack_anim) = if let Some(attack) = attack_state {
        (attack.attack_count, attack_animation(attack).clip_name().to_string())
    } else {
        (0, String::new())
    };
    ...
    let update = PendingOutboundUpdate {
        ...
        send_at: if lag.outbound_delay_ms == 0 && lag.packet_loss_chance == 0.0 {
            ...
            web_time::Instant::now() + std::time::Duration::from_millis(lag.outbound_delay_ms)
        },
    };
